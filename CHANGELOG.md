...
### Changed
- **BREAKING — access and refresh tokens are now standard HS256 JWTs** (`token.go`): tokens are emitted as `base64url(header).base64url(claims).base64url(signature)` with a JOSE header of `{"alg":"HS256","typ":"JWT"}`, replacing the previous two-segment `payload.signature` format. Tokens issued before the upgrade will not validate. Verification enforces an explicit `HS256` allow-list, so `alg: none` and algorithm-confusion tokens are rejected. Access and refresh tokens now also carry `email`, `role`, `isEmailVerified` and `isTotpEnabled` claims
