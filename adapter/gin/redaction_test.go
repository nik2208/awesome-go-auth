...
func TestMount_Register_RedactsCredentialMaterial(t *testing.T) {
	...
	body, _ := json.Marshal(map[string]string{"email": "ginredact@example.com", "password": "password1", "tenantId": "t1"})
	...
	assertCamelCaseUser(t, rr.Body.Bytes(), "ginredact@example.com")
}
...
func TestMount_Login_RedactsStoredPasswordHash(t *testing.T) {
	...
	user, _, err := a.Register(ctx, auth.RegisterInput{Email: "ginredactlogin@example.com", Password: "password1", TenantID: "t1"})
	if err != nil {
	...
	r := buildRouter(a)
	body, _ := json.Marshal(map[string]string{"email": "ginredactlogin@example.com", "password": "password1", "tenantId": "t1"})
	...
	assertNoCredentialMaterial(t, rr.Body.Bytes())
	assertCamelCaseUser(t, rr.Body.Bytes(), "ginredactlogin@example.com")
}
